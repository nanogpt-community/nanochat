...
export const mcpToolDefinitions: ChatCompletionTool[] = [
	// Balance / Usage
	{
		...
		function: {
			...
			description:
				'Check the current account balance (USD and Nano). Use before expensive operations to verify sufficient funds.',
			parameters: {
				type: 'object',
				properties: {},
				required: [],
	...
	// Web Search
	{
		...
		function: {
			...
			description:
				'Search the web for current information. Use this when you need up-to-date information, recent news, or to verify current facts.',
			parameters: {
				type: 'object',
				properties: {
					query: {
					...
					depth: {
						...
						enum: ['standard', 'deep'],
						description:
							'Search depth - standard for quick results, deep for more comprehensive search',
					},
				},
				required: ['query'],
			},
	...
	// URL Scraping
	{
		...
		function: {
			...
			description:
				'Scrape content from one or more web URLs. Use this when you need to read the full content of a specific webpage.',
			parameters: {
				type: 'object',
				properties: {
					urls: {
	...
	// YouTube Transcription
	{
		...
		function: {
			...
			description:
				'Get the transcript of a YouTube video. Use this when asked about YouTube video content.',
			parameters: {
				type: 'object',
				properties: {
					url: {
	...
	// Image Generation
	{
		...
		function: {
			...
			description:
				'Generate images from a text prompt using AI image models like DALL-E, Flux, or Midjourney.',
			parameters: {
				type: 'object',
				properties: {
					prompt: {
	...
	// Vision / Image Analysis
	{
		...
		function: {
			name: 'nanogpt_vision',
			description:
				'Analyze images with a vision-capable model. Use this to describe, analyze, or extract information from images.',
			parameters: {
				type: 'object',
				properties: {
					prompt: {
					...
					image_url: {
						type: 'string',
						description:
							'URL of the image to analyze. If omitted, the most recent image in the chat will be used.',
					},
					model: {
						type: 'string',
...
async function executeGetBalance(
	...
	if (!response.ok) {
		...
		return {
			success: false,
			result: '',
			error: `Balance check failed: ${response.status} ${errorText}`,
		};
	}
...
async function executeScrapeUrls(
	...
	if (!response.ok) {
		...
		return {
			success: false,
			result: '',
			error: `URL scraping failed: ${response.status} ${errorText}`,
		};
	}
...
async function executeYoutubeTranscribe(
	...
	if (!response.ok) {
		...
		return {
			success: false,
			result: '',
			error: `YouTube transcription failed: ${response.status} ${errorText}`,
		};
	}
...
async function executeListModels(
	...
	if (!response.ok) {
		...
		return {
			success: false,
			result: '',
			error: `List ${type} models failed: ${response.status} ${errorText}`,
		};
	}
...
async function executeListAudioModels(
	...
	if (!response.ok) {
		...
		return {
			success: false,
			result: '',
			error: `List audio models failed: ${response.status} ${errorText}`,
		};
	}
...
async function executeVision(
	...
	if (!response.ok) {
		...
		return {
			success: false,
			result: '',
			error: `Vision analysis failed: ${response.status} ${errorText}`,
		};
	}
...
/**
 * Models that answered a tools-attached request with a definitive "no".
 *
 * NanoGPT's catalog exposes `capabilities.tool_calling`, but it's wrong in both
 * directions — zai-org/glm-5.2 advertises `true` and still returns 400
 * `tool_choice_unsupported` — so the only trustworthy signal is the API's own
 * answer. Remember it so we pay the failed round trip once per model, not once
 * per message.
 *
 * ponytail: per-process Set, cleared on restart. Persist it if the retry cost
 * ever shows up in traces.
 */
const modelsRejectingTools = new Set<string>();

export function modelRejectsTools(modelId: string): boolean {
	return modelsRejectingTools.has(modelId);
}

/**
 * Did this request fail *because* we attached tools? Only a definitive answer
 * is remembered — a 503 from a flaky upstream provider is not evidence that the
 * model can't do tool calls.
 */
export function noteToolsRejection(modelId: string, error: unknown): void {
	const msg = String(error instanceof Error ? error.message : error).toLowerCase();
	const definitive =
		msg.includes('tool_choice_unsupported') ||
		(msg.includes('tool') && (msg.includes('not support') || msg.includes('unsupported')));

	if (definitive) modelsRejectingTools.add(modelId);
}
