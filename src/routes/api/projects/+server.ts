...
export async function GET({ request }: RequestEvent) {
	...
	const ownedProjects = await db.query.projects.findMany({
		...
		with: {
			// Only the metadata the project list renders. `files: true` returned
			// extractedContent — the full text of every attached PDF — for every
			// project, on a request the sidebar makes on load. Prompt construction
			// reads extractedContent separately.
			files: {
				columns: { id: true, projectId: true, fileName: true, fileType: true, createdAt: true },
			},
			members: {
				with: {
					user: {
